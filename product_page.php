...
//$sql = "SELECT * FROM arancini;";
$sql = 'SELECT prezzo, gusto as gus, descrizione, immagine, descrizione, (SELECT sum(quantita) FROM stock WHERE gusto=gus) as disponibilita FROM arancini';
...
<head>
  ...
  <!-- Basic -->
  <title>SOLOarancini | Prodotti</title>
...
$(document).ready(function(){
		<?php $_SESSION['carrello'] = array_unique($_SESSION['carrello']); 
			array_filter($_SESSION['carrello']);?>
var selezionati = <?php echo json_encode($_SESSION['carrello']) ?>;
for (id in selezionati){
			var selector = "#"+selezionati[id];	
			$(selector).text("Nel carrello");
...
function aggiungi(id){		
	$.post("product_page.php", { item: id }); 
	<?php array_push($_SESSION['carrello'], $_POST['item']);?>
        ...
        <div class="container">
          ...
          <div class="navbar-collapse collapse">
            ...
            <ul class="nav navbar-nav navbar-right">
              ...
              <li><a class="active" href="product_page.php">Prodotti</a>
              </li>
              <li>
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <div class="big-title text-center">
              <h1>Abbiamo degli ottimi prezzi per <strong>te!</strong></h1>
            </div>
            <!-- End Big Heading -->

            <!-- Text -->
            <p class="text-center" style="margin-bottom:20px;">Qui è possibile sciegliere il gusto dell'arancino che si desidera acquistare. Abbiamo un vasto assortimento di gusti che vanno dai classici fino ai tradizionali.</p>
          </div>
        </div>
	...
	<?php while($row = $result->fetch_assoc()){?>
          ...
          <div class="col-md-3 col-sm-3 col-xs-12" style="margin-bottom: 4%;">
            <div class="pricing-table">
              <div class="plan-name">
                <h3><?php echo $row['gus']?></h3>
              </div>
              <div class="plan-price">
                <img src="images/<?php echo $row["immagine"] ?>" alt="#" style="width:128px;height:128px;">
              </div>
	      <div>
		...
		<div class="plan-list">
                <ul>
                  <li><strong><?php echo $row["prezzo"] ?>€</strong></li>
                  <li><strong><?php if(!isset($row["disponibilita"])){echo "Prodotto non disponibile";}else{echo "Quantità: ".$row["disponibilita"];}?></strong></li>
                </ul>
              </div>
              <div class="plan-signup">
              <?php if(isset($row["disponibilita"])){ ?><button class="btn-system btn-small"id=<?php echo $row['gus'] ?> onclick="aggiungi(this.id)">Compra ora!</button> <?php } ?>
              </div>
            </div>
          </div>
	<?php  } ?>
