...
<style type="text/css">
  ...
  <!-- Basic -->
  <title>SOLOarancini | Profilo</title>
...
<body>
  ...
  <div id="container">
	...
	<?php if ($_SESSION["login"]) {?> <div class="loginform" style="width: 30%; margin: 0 auto; margin-bottom: 3%; margin-top: 3%;">
		<form name="Registrazione" action="modifica_dati_utente.php" method="post" accept-charset="utf-8" class="subscribe">
			<label for="firstname">Nome</label>
			<input id="firstname" type="text" name="nome" <?php echo "value= '".$_SESSION['nome']."'" ?> required>
			<label for="lastname">Cognome</label>
			<input id="lastname" type="text" name="cognome" <?php echo "value= '".$_SESSION['cognome']."'" ?> required>
	<?php if(!$_SESSION['role']) { ?><label for="address">Indirizzo</label>
			<input id="address" type="text" name="indirizzo" <?php echo "value= '".$_SESSION['indirizzo']."'" ?> required>
                        <label for="city">Città</label>
			<input id="city" type="text" name="citta" <?php echo "value= '".$_SESSION['citta']."'" ?>  required> <?php } ?>
			<label for="email">Email</label>
			<input id="email" type="email" name="mail" <?php echo "value= '".$_SESSION['mail']."'" ?> required>
			<label for="phone">Numero telefonico</label>
			<input id="phone" type="text" name="telefono" <?php echo "value= '".$_SESSION['telefono']."'" ?> required> 
			<input name="registered" type="submit" value="Aggiorna le tue info">
		</form></br>
		<?php if(isset($_SESSION['risultato'])){ echo $_SESSION['risultato']; 
				                         unset($_SESSION['risultato']);} ?>
	</div> <?php } else { ?> <div class="loginform" style="width: 30%; margin: 0 auto; margin-bottom: 10%; margin-top: 10%;">
	<h3>Non hai effettuato il <a href="login_page.php">login</a>  </h3>
	</div> <?php } ?>
